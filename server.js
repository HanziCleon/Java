// server.js - DongTube (Vercel Compatible)
// Features:
// - Color console logs
// - GitHub database.json auto-update using SHA cache
...
// Vercel doesn't support persistent file system, use memory only
let animeCache = {
  sha: null,
  data: [],
  lastFetched: null
  ...
  try {
    if (!animeCache || !animeCache.data || animeCache.data.length === 0) {
      await new Promise((resolve) => {
        app._router.handle({ method: 'GET', url: '/api/anime', headers: {} }, {
          json: (data) => { animeCache.data = data; resolve(); },
          status: () => ({ json: (data) => resolve() })
        });
      }).catch(() => {});
    }
